...
class DiffieHellmanApp:
    def __init__(self, root):
        ...
        self.p_entry = tk.Entry(left_frame, width=15, font=("Arial", 10))
        ...
        self.g_entry = tk.Entry(g_frame, width=10, font=("Arial", 10))
        ...
        self.a_entry = tk.Entry(alice_frame, width=15, font=("Arial", 10))
        ...
        self.b_entry = tk.Entry(bob_frame, width=15, font=("Arial", 10))
    ...
    def show_primitive_roots(self):
        ...
        if not roots:
            messagebox.showinfo("Результат", f"Для p = {p} нет первообразных корней.")
            return
    ...
    def select_g(self, g, window):
        ...
        self.log(f"Выбран g = {g} (первообразный корень)")
        window.destroy()
    ...
    def random_key(self, entry):
        try:
            ...
            if p > 3:
                ...
                entry.insert(0, str(random.randint(2, p - 2)))
            else:
                messagebox.showwarning("Предупреждение", "p слишком маленькое, сгенерируйте большее число.")
        except ValueError:
            messagebox.showerror("Ошибка", "Сначала введите корректное p!")

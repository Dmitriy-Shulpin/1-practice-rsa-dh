...
def calculate_e():
    ...
    # Сохраняем в глобальные переменные для второго шага
    ...
    # Убираем возможность ввода в поле e (если оно было заблокировано)
...
# =============================================
def run_encrypt():
    # Очищаем поле вывода (но сохраняем предыдущие расчеты, если надо)
    # Чтобы не потерять шаг 1, мы будем дописывать в конец
    out.insert(tk.END, "\n" + "=" * 60 + "\n")
    ...
    # Проверяем, что был выполнен шаг 1
    ...
    if e not in current_possible_e:
        messagebox.showerror(
            ...
            f"e должно быть:\n"
            f"1. 1 < e < {current_phi}\n"
            f"2. Взаимно просто с {current_phi}\n\n"
    ...
    if e == d:
        messagebox.showerror(
            "Ошибка",
            f"ВНИМАНИЕ! e = d = {e}\n\n"
            "Ключи совпадают! Это небезопасно.\n"
            "Пожалуйста, выберите другое e из списка:\n"
            f"{current_possible_e}"
        )
        return
    ...
    # Определяем режим: число или текст
        ...
        for ch in msg:
            ...
            if code >= current_n:
                messagebox.showerror(
                    ...
                    "Сообщение слишком длинное для данного n.\n"
...
tk.Label(root, text="Пример: p=43, q=59 → Допустимые e: список (много значений)", font=("Arial", 9), fg="gray").pack()

# Поле вывода
tk.Label(root, text="Результаты:", font=("Arial", 12, "bold")).pack(pady=5)

out = scrolledtext.ScrolledText(root, width=85, height=23, font=("Courier New", 10))
out.pack(padx=20, pady=10)
